...
class LessonModule {
  ...
  static async findByTeacher(teacherId, db = pool) {
    const query = `SELECT * FROM vws_lessons WHERE lesson_teacher=$1`;
    ...
    try {
      ...
      console.error(
        `Error in findByTeacher: Could not retrieve lessons for teacher ID ${teacherId}.`,
        error,
      );
      throw new Error(`Database error: Failed to fetch lessons by teacher ID ${teacherId}.`);
    }
  }

    static async findByTeacherAndName(teacherId, name, db = pool) {
    const query = `SELECT * FROM vws_lessons WHERE lesson_teacher=$1 AND lesson_name ILIKE $2`;
